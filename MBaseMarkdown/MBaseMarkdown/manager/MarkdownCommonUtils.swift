...
class MarkdownCommonUtils: NSObject {
    ...
    //获取两个NSRange的最大区间
    static func getMaxRange(_ range:NSRange, otherRange:NSRange) -> NSRange {
    ...
    static func isContainInCodeKey(_ string: NSString, codeKey: String, range: NSRange) -> Bool {
        ...
        if string.isExistString(codeKey as NSString, range: backRange) {
            return false;
        }
        return true;
    }
    ...
    //腐蚀制定ranges
    static func corrodeString(_ ranges: [NSRange], corrodeRanges: [NSRange]) -> [NSRange]{
        ...
        holeRanges.append(contentsOf: corrodeRanges);
        holeRanges = holeRanges.sorted(by: { r1, r2 in r1.location < r2.location })
